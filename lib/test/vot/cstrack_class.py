...
class cstrack(object):
    def __init__(self, tracker_name='', para_name=''):
        ...
        params.checkpoint = os.path.join(env_settings().checkpoints_path, env_settings().ckpt_name)
...
def run_vot_exp(tracker_name, para_name, vis=False, out_conf=False, channel_type='color'):
    ...
    if vis and (not os.path.exists(save_root)):
        os.mkdir(save_root)
    tracker = cstrack(tracker_name=tracker_name, para_name=para_name)
