    ...
    def getitem(self):
        ...
        while not valid:
            ...
            try:
                ...
                # transform rgb to rgb+x
                if template_frames[0].shape[-1] == 3:
                    frames_update = []
                    for item in template_frames:
                        frames_update.append(cv2.merge((item, item)))
                    template_frames = frames_update

                    frames_update = []
                    for item in search_frames:
                        frames_update.append(cv2.merge((item, item)))
                    search_frames = frames_update
